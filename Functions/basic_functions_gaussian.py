...
from .basic_functions_bayesian import compute, compute_single, run_Metropolis, langevin_sampling
...
def sample_and_group(my_energy_function, *,
    ...
    i_print = 2000, seed = 1, kT = 1., if_tqdm = False,
    # parameters for the Group_points function:
    tolerance = 0.05, if_diff = True, threshold = 50, value = None):
    """
    Sample with Metropolis algorithm `run_Metropolis` the energy landscape described by `my_energy_function`.
    Then, group the energy values by the consecutive frames in which the energy takes approx. the same value.
    
    sampling_pars includes: starting_point, dx (for Metropolis), which_sampling ('Metropolis' or 'Langevin'),
    n_steps (seed might be included in `sampling_pars` since it refers only to the sampling algorithm)

    group_pars includes: tolerance, if_diff, threshold
    tolerance = 0.1
    if_diff = True
    threshold = 25

    You can pass the arguments as, for example:
    sample_and_group(my_energy_function_simple, **sampling_pars, seed=seed, **group_pars, tolerance=tolerance)
    where sampling_pars and group_pars are dictionaries
    """

    assert (which_sampling == 'Metropolis') or (which_sampling == 'Langevin'), 'error in which_sampling'
    if which_sampling == 'Metropolis': assert dx is not None, 'error in dx'

    if which_sampling == 'Metropolis':
        out_sampling = run_Metropolis(starting_point, dx, my_energy_function, n_steps=n_steps,
            i_print=i_print, seed=seed, kT=kT, if_tqdm=if_tqdm)
